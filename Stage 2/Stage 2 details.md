...
* Each coach in `coaches_24_25.csv` **has** a GameRecord of their stats based on Current(This season), Franchise(seasons with the current team), and Overall(career totals in the NBA). Coaches in the NBA will have these game records displayed after seasons. 

* The has relationship is one-to-one since each coach will have at most one game record. This is because all NBA games they have coached are in this one record of all time wins, losses and games played. A game record has 1 coach that it is tied to since we do not keep track of a record for multiple coaches that would logically not make any sense. 
...
* The primary key in this relationship is (CoachID and TeamID). This will uniquely identify every team from each other.
...
* This relationship is going to be a many-to-many relationship since each team in the NBA does play many games and each game is played by many teams which it is actually 2 games no more or less. As you can see by the `NBA_24_25_schedule.csv`.
...
* In the newly added dataset `Arena.csv` will connect a team owning an arena by their team name. In NBA a team can definitely own a specific arena and the dataset does reflect this.
...
* The primary key for this owns relationship will be (Team name and Arena name) it is safe to assume that there will never be a team name identical to another one in the NBA and an arena name will never be named the same as another in the NBA as well. 
...
* The primary key in this relationship will be (Team_ID and Player_ID and Season) to uniquely identify every entity in the database. We will assume that a player cannot get drafted multiple times in a season if we choose this primary key.
...
* A player in the NBA will have player information about their own biological metrics and other personal properties of themselves. Thus why this relationship exists

* The has relationship is one-to-one since every player will have only player information about themselves assigned to them and player information will be assigned to only the player that it coorsponds to and no one else.
